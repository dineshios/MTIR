...
@interface MMRotatedView()

@property (nonatomic,assign) NSInteger selectedIndex;
- (void)timerStop;
- (void)tapStopped;
- (void)decelerate;
@property (strong, nonatomic) MMRenderView *pieChart;
@property (nonatomic, assign) BOOL canLayerOpen;
@end

@implementation MMRotatedView
...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        self.pieChart = [[MMRenderView alloc]initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height)];
        self.pieChart.dataSource = self;
        [self.pieChart setStartPieAngle:0];
        [self.pieChart setAnimationSpeed:1.0];
        //        [self.pieChart setLabelFont:[UIFont fontWithName:@"DBLCDTempBlack" size:15]];
        [self.pieChart setLabelFont:[UIFont fontWithName:@"DBLCDTempBlack" size:15]];//改变font大小
...
- (void)drawRect:(CGRect)rect {
    ...
    if (wedges > [mColorArray count]) {
        NSLog(@"Number of colors is not enough: please add %zi kinds of colors.",wedges - [mColorArray count]);
...
/**
 *  rotate动画停止后的居中调整
 *  after rotate animation, get selected slider in middle.
 */
- (void)delayAnimate
{
    ...
    [UIView animateWithDuration:0.42 animations:^{
        ...
        //在文字上也添加旋转的效果，保持水平状态，别忘记在decelerate和tap等方法里也要保持文字的状态
        for(MMSlideLayer *layer in [self.pieChart.pieView.layer sublayers]){
            ...
            CGAffineTransform rotateTransform0 =  CGAffineTransformMakeRotation(rotateTheta);
            
            if (layer.isTouchMoved) {
                
                isToRotate = YES;
                rotateTransform = CGAffineTransformRotate(layer.layerTransform,   - 1 * rotateTheta) ;
                
            }else{
                rotateTransform = CGAffineTransformRotate(rotateTransform0,   - 2 * rotateTheta) ;
            }
            
            if (isToRotate) {
                
                for (CATextLayer *textLayer in (CATextLayer *)[layer sublayers]) {
                    
                    [textLayer setAffineTransform:rotateTransform];
                }
                layer.isTapped        = NO;
                layer.isTouchMoved    = NO;
            }
        }
...
- (void)tapStopped {
    ...
    if (tapAreaIndex != self.selectedIndex) {
        ...
        for(MMSlideLayer *layer in [self.pieChart.pieView.layer sublayers]){
            ...
            CGFloat angle = -[self rotationThetaForNewTheta:K_POINTER_ANGLE];
...
- (void)decelerate {
    ...
    for(MMSlideLayer *layer in [self.pieChart.pieView.layer sublayers]){
        ...
        CGFloat angle = -[self rotationThetaForNewTheta:mAbsoluteTheta];
        CGAffineTransform rotateTransform = CGAffineTransformMakeRotation(angle);
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    //在文字上也添加旋转的效果，保持水平状态，别忘记在停止动画的decelerate方法里也要保持文字的状态
    for(MMSlideLayer *layer in [self.pieChart.pieView.layer sublayers]){
        
        // to make textLayer horizontal, the angle need to be opposite.
        CGFloat angle = -[self rotationThetaForNewTheta:mAbsoluteTheta];
        CGAffineTransform rotateTransform = CGAffineTransformMakeRotation(angle);
        ...
        layer.isTouchMoved    = YES;
        //set layer`s Transform
        layer.layerTransform        = rotateTransform;
